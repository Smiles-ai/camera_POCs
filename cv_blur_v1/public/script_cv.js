...
// function exposure_detect(mat, dark_thres=0.3, bright_thres=0.15){
...
//     let low_bright = new cv.Mat(dst.rows, dst.cols, dst.type(), [220,220,220,0]);
...
function put_text_blur(blur_status, image){

    coordinates_blur = new cv.Point(30, 30);

    if (blur_status== "Not Blurry"){
        // console.log("I am inside not blurry")
        cv.putText(image, blur_status, coordinates_blur, cv.FONT_HERSHEY_SIMPLEX, .7, [0,255,0,255], 2)
        }
    else{
        // console.log("I am inside blurry")
        cv.putText(image, blur_status, coordinates_blur, cv.FONT_HERSHEY_SIMPLEX, .7, [255,0,0,255], 2)
